...
class H2HolyStrength: HXDDPowerupSphere {
    ...
    override void OnNeutralPickup(Actor toucher) {
        ...
        // Give Health
        int amountHealth = floor((1000.0 / TICRATEF) / tickDuration);
        HealThing(amountHealth);

        // Give Ammo
        int amount = floor((1000.0 / TICRATEF) / tickDuration);
...
class H2SoulSphere: HXDDPowerupSphere {
    ...
    override void OnEvilPickup(Actor toucher) {
        ...
        // Give Health
        int amountHealth = floor((1000.0 / TICRATEF) / tickDuration) * 2;
        HealThing(amountHealth);

        // Give Ammo
        int amount = 15 + floor((1000.0 / TICRATEF) / tickDuration);
        self.GiveAmmoAmount(toucher, amount);
    }
